...
public partial class AnimaHomePage : Page {
    ...
    public AnimaHomePage() {
        ...
        if (loadedCount == 0) {
            startButton.IsEnabled =
                false;  // disable button before the backend is ready
        ...
        if (Directory.Exists(path)) {
            string[] directoriesWithinPath =
                Directory.GetFiles(path, "*.animaprofile");
            if (directoriesWithinPath.Length > 0) {
                ...
                if (File.Exists(path + "/" + userName + ".animaprofile")) {
                    ...
                    string spaces = String.Concat(
                        Enumerable.Repeat(" ", 52 - userNameLength));
                    desc.Inlines.Clear();
                    desc.Inlines.Add(
                        new Run(spaces + "Welcome Back " + userName +
            ...
            List<LanguageItem> languageItems =
                new List<LanguageItem>  // language dropdown list sources
                {
                    new LanguageItem(
                        ...
                        "English"),
                    new LanguageItem(
                        "Images\\icons\\country flag\\france_640.png",
                        "French"),
                    new LanguageItem(
            ...
            MessageBox.Show("Error: Missed animaProfiles directory", "Error",
    ...
    private void InitialiseBackendJson()  // used to initialise all values in
                                          // backend json file
    {
    ...
    private void InitialiseFrontendJson() {
        ...
        frontendJsonContent = JsonConvert.SerializeObject(frontendJsonObject,
                                                          Formatting.Indented);
