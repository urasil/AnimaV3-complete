...
namespace dotnetAnima.Core {
    public class AudioRecorder {
        ...
        public void StartRecording(string outputPath) {
            ...
            waveIn.WaveFormat =
                new WaveFormat(44100, 1);  // 44.1 kHz, 16-bit mono
            ...
            try {
                ...
                MessageBox.Show($"Error starting recording: {e.Message}",
        ...
        public void StopRecording() {
            try {
                waveIn?.StopRecording();  // if waveIn != null, then
                                          // waveIn.StopRecording()
            } catch (Exception e) {
                Console.WriteLine($"Error stopping recording: {e.Message}");
                MessageBox.Show($"Error stopping recording: {e.Message}",
                                "Error", MessageBoxButtons.OK,
                                MessageBoxIcon.Error);
            } finally {
                CleanUp();
        ...
        public void PlaySound() {
            if (File.Exists(audioPath)) {
                try {
                    ...
                    MessageBox.Show($"Error playing sound: {e.Message}",
                                    "Error", MessageBoxButtons.OK,
                                    MessageBoxIcon.Error);
                }
            } else {
                Console.WriteLine("Audio file does not exist");
                MessageBox.Show($"Audio file does not exist", "Error",
        ...
        public void StopSound() {
            try {
                ...
                MessageBox.Show($"Error stopping sound: {e.Message}", "Error",
                                MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
