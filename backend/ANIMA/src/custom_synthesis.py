...
def generateAnimaProfile(
    ...
    """Create voice model for synthesis and save to a file.
    ...
    Args:
        ...
        file_path (str):
            Destination file path to save profile to.
...
#   save data to file
    toSave = [model, CONFIG, speaker_id, style_mel, style_text, d_vector, language_id]

    with open(file_path, 'wb') as fp:
            pickle.dump(toSave, fp)
            print('Generated ANIMA profile')

def synthesisFromProfile(
    profile_path : str,
    text : str,
    use_cuda,
    do_trim_silence = False,
):
    """Synthesize voice for the given text using Griffin-Lim vocoder using a saved ANIMA Profile.

    Args:
        profile_path (str):
            The string path to the ANIMA profile.
        ...
        do_trim_silence (bool):
            trim silence after synthesis. Defaults to False.
    """
    with open(profile_path, 'rb') as fp:
        model, CONFIG, speaker_id, style_mel, style_text, d_vector, language_id = pickle.load(fp)
...
def synthesis(
    ...
    # toSave = [model, speaker_id, style_mel, style_text, d_vector, language_id]
    # print(toSave)

    # with open("C:/Users/Pallav/Desktop/workspace/ANIMA/v1/ANIMA/testfile.txt", 'wb') as fp:
    #         pickle.dump(toSave, fp)
    #         print('Done writing list into a binary file')

    with open("C:/Users/Pallav/Desktop/workspace/ANIMA/v1/ANIMA/testfile.txt", 'rb') as fp:
            model, CONFIG, speaker_id, style_mel, style_text, d_vector, language_id = pickle.load(fp)
